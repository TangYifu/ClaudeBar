...
- 🔥 **今日 Token 消耗统计**：
  ...
  - **全方位细分**：
    - **生成输出**：大模型实际生成的输出 Token 数（如 `515K`）。
    - **首次载入缓存**：初次写入 Prompt Caching 的 Token 数（如 `3.0M`）。
    - **缓存命中读取**：从 Prompt Caching 重复读取的 Token 数（如 `52.0M`）。
    - **缓存命中率**：缓存读取占全部输入的比例（如 `94.5%`）。
    - **交互消息**：统计当天的交互轮次（如 `516条`）。
