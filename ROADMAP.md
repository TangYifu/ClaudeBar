...
- 使用原生 Swift 与 SwiftUI 开发，无需 Electron、Python 或 Node.js 等额外运行时。
...
- **v1.2：性能、可靠性与正式发布（当前阶段）**
  - [x] 支持今天、昨天和本周切换。
  - [x] 增加项目级与模型级消耗排行。
  - [x] 增加额度阈值预警通知。
  - [x] 使用文件偏移量增量扫描 JSONL，并持久化统计缓存。
  - [x] 持久化最后成功配额，支持 HTTP 429 `Retry-After` 退避。
  - [x] 嵌入应用版本、构建号和 Git commit。
  - [x] 建立统计、缓存和构建自动化测试。
  - [x] 打标签自动创建 Universal 2 GitHub Release。
  - [ ] 增加本月与自定义时间范围。
